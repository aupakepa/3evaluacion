...
public class Renfe {
	...
	private static List<Billete> leeFichero(Map<Fecha, List<Tren>> viajes, Tren[] trenes) {
		...
		if (f.exists()){
			...
			for (Billete e: billetes){
				if (maxContador<e.getNumeroBillete()){
					maxContador=e.getNumeroBillete();//guardamos el número de billete mayor
				}
				if (!viajes.containsKey(e.getFecha())){//si la fecha no está en el mapa...
					trenesFecha=new ArrayList<>();//Crea una Lista nueva para el nodo del mapa
					tren= new Tren(estaTren(trenes, e.getNumeroTren()));//crea un tren para esa fecha basado en el que se ha escogido.
					tren.setAsiento(e.getVagon(), e.getAsiento());//reserva el asiento en el vagón correspondiente
					trenesFecha.add(tren);//añade el tren a la Lista
				} else {// si la fecha ya está en el mapa...
					trenesFecha=viajes.get(e.getFecha());//obtiene el Lista de trenes de esa fecha que está en el mapa
					Tren trenAux=estaTren(trenesFecha, e.getNumeroTren());// busca el tren (númeroTren) en la Lista
					if (trenAux==null){//Si no está...
						tren= new Tren(estaTren(trenes, e.getNumeroTren()));//crea un tren para esa fecha basado en el que se ha escogido.
						tren.setAsiento(e.getVagon(), e.getAsiento());//reserva el asiento en el vagón correspondiente
						trenesFecha.add(tren);//añade el tren a la Lista
				...
				viajes.put(e.getFecha(), trenesFecha);//añade-sustituye el nodo al mapa
			}
