...
public class GestionMueble {
	...
	public static void main(String[] args) {
		...
		Leer.mostrarEnPantalla("-------");
		localizaAlmacen(almacenes,muebles);
	...
	public static void inicializa(Almacen[] almacenes, Mueble [] muebles){
		...
		for(int i=0; i<15; i++){
			...
			muebles[i].setAlmacen(almacenes[numeroAlmacen]);
		}
	}
	...
	public static void localizaAlmacen(Almacen [] almacenes,Mueble [] muebles){
		for(int i= 0; i<almacenes.length; i++){
			Leer.mostrarEnPantalla(almacenes[i].muestraAlmacen());
			for (int j= 0; j<muebles.length;j++){
				if (almacenes[i]==muebles[j].getAlmacen()){
